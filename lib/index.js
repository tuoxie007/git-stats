...
GitStats.prototype.codes = function (options, callback) {
    ...
    var before = options.start.format(DATE_FORMAT);
    var after = options.end.format(DATE_FORMAT);
    var out = Spawn("git", [
        "--no-pager", 
        "log", 
        "--since", 
      ...
      for (var i = 0; i < lines.length; i++) {
          ...
          if (line.length) {
            if (line.indexOf('file') > 0) {
                ...
                line = line.replace(' file changed', '');
                ...
                var changed = 0;
                if (nums.count > 2) {
                    changed = nums[1] + nums[2];
                } else {
                    changed = nums[1];
                }
                if (!changes[today]) {
                    changes[today] = parseInt(changed);
                } else {
                    changes[today] += parseInt(changed);
                }
            } else {
                today = Moment(line).format(DATE_FORMAT);
            }
          }
      }
    }).on('close', function (code) {
        Object.keys(changes).forEach(function (c) {
            cal.push([c, Math.log(changes[c])])
        });
        var data = {
            theme: options.theme
          , start: options.start
